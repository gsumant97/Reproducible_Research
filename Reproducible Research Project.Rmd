...
2. Histogram of the total number of steps taken eachd ay
...
```{r}
setwd("C:/Users/91739/Downloads/RProg_Practice/Coursera/Reproducible_Research")
...
head(activity)
str(activity)
#Total number if missing data
sum(is.na(activity$steps))/dim(activity)[[1]]
#Transforming the data column into data format using lubridate
library(lubridate)
...
**Step 2: **
 Histogram of the total number of steps taken each dat
```{r, echo=TRUE}
library(ggplot2)
...
qplot(Q2$`Total Steps`, geom = "histogram", xlab = "Total Steps", ylab = "Counts", main = "Total StepsHistogram")
png("plot1.1.png")
qplot(Q2$`Total Steps`, geom = "histogram", xlab = "Total Steps", ylab = "Counts", main = "Total StepsHistogram")
dev.off()
```
**Step 3: **
 Mean and Median number if the steps taken each day
...
**Step 5: **
The 5-Minute interval that, on average, contain the maximum number of steps
...
**Step 7: **
Histogram of the total numbers of steps taken each day after missing values are imputed

```{r, echo=TRUE}
png("plot7.png")
qplot(Q6.6$Steps, geom = "histogram", main = "Total Step taken histogram post imputation", xlab = "Steps", ylab = "Count")
dev.off()
qplot(Q6.6$Steps, geom = "histogram", main = "Total Step taken histogram post imputation", xlab = "Steps", ylab = "Count")
